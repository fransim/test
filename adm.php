...
class Adm extends CI_Controller {
	...
	public function hasil_ujian_cetak() {
		...
		/*$a['hasil'] = $this->db->query("SELECT m_siswa.nama, tr_ikut_ujian.list_soal, tr_ikut_ujian.list_jawaban, tr_ikut_ujian.tgl_mulai, tr_ikut_ujian.tgl_selesai
										FROM tr_ikut_ujian
										INNER JOIN m_siswa ON tr_ikut_ujian.id_user = m_siswa.id
										WHERE tr_ikut_ujian.id_tes = '$uri3'")->result();
		$a['hasil'] = $this->db->query("SELECT tr_ujian.id_user, tr_ujian.id_soal, tr_ujian.jawaban
										FROM tr_ujian
										INNER JOIN m_siswa ON tr_ujian.id_user = m_siswa.id
										WHERE tr_ikut_ujian.id_tes = '$uri3'")->result();
		$a['hasil'] = $this->db->query("SELECT m_siswa.nama, m_siswa.id
										FROM tr_ikut_ujian
										INNER JOIN m_siswa ON tr_ikut_ujian.id_user = m_siswa.id
										WHERE tr_ikut_ujian.id_tes = '$uri3'")->result();*/
		$a['hasil'] =  $this->db->query("SELECT m_siswa.id, m_siswa.nama, m_soal.id_soal, m_soal.jawaban
										FROM tr_ujian
										INNER JOIN m_siswa ON tr_ujian.id_user = m_siswa.id_user
										INNER JOIN m_soal ON tr_ujian.id_soal = m_soal.id
										INNER JOIN m_soal ON tr_ujian.jawaban = 
										where tr_ujian.id_tes = '$uri3'")->result();
		$a['controller']=$this; 
		$this->load->view('m_guru_tes_hasil_detil_cetak', $a);
	}
	
	public function soal_jawaban($id_mahasiswa)
	{
		$a['result'] = $this->db->query("SELECT list_soal, list_jawaban, tgl_mulai, tgl_selesai
										FROM tr_ikut_ujian
										WHERE id_user = '$id_mahasiswa'")->result();
		//$this->load->view('m_guru_tes_hasil_detil_cetak', $a);
		//echo $id_mahasiswa
		return $a;
	}
